...
<form action="" method="post">
          <div class="center">
                 <?php 
    $sql = "SELECT * FROM podp WHERE dp_order = $po AND dp_site = $site_cd";
    ...
    if ($result->num_rows > 0) {
        ...
        <?php }}?>
              ...
              <p1>Mode of Transmission: </p1> <select id="trans" onchange="showEmail()" name="po_tr" required>
                <option></option>    <?php 
    $sql = "SELECT * FROM potr ";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            $state = $row["st_state"];
            ?>
          <option <?php if ($row["tr_edi"] ==0) {echo "hidden";} if($row["su_trans"] =='EDI') {echo "selected";} ?>  value="EDI">EDI (Electronic Data Interchange)</option>    
          <option <?php if ($row["tr_email"] ==0) {echo "hidden";} if($row["su_trans"] =='email') {echo "selected";}?> value="email">Email</option>    
          <option <?php if ($row["tr_efax"] ==0) {echo "hidden";} if($row["su_trans"] =='efax') {echo "selected";}?> value="efax">E-fax</option>    
...
<?php 
    ...
    $o = $_GET["o"];
    //GET SUM OF ORDER
    $sql1 = "SELECT sum(ln_qty*ln_price) AS total FROM poln WHERE ln_order = $o AND ln_wh = $site_cd";
