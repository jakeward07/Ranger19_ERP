...
<head>
    ...
    <title>Change Background Image - RANGER 5</title>
    </head>
<body>
...
<div class="center">
    ...
    <p1>Background Image: </p1><input accept="image/png, image/gif, image/jpeg" type="file" id="image" name="uploadedFile" style="background:none"><br>
    ...
    <button type="submit" name="submit" class="submit">Save Settings &rarr;</button>
      <?php if (!empty($bgimg)) {?> <br><br><a href="sf_bgimg.php?delete=true"><button type="button" class="submit">Delete Background Image &rarr;</button>   </a><?php }?>
...
$allowedfileExtensions = array('gif','png', 'jpg', 'jpeg');
...
{
    $sql = "SELECT us_bgimg FROM usmf WHERE us_user = '$usernm'";
  ...
  $sql = "UPDATE usmf SET us_bgimg = '$newFileName', us_bgcol = '$_POST[color]' WHERE us_user = '$usernm'";
    $conn->query($sql);
    echo $conn->error;
}
}}
else {
    $sql = "UPDATE usmf SET us_bgcol = '$_POST[color]' WHERE us_user = '$usernm'";
    $conn->query($sql);
}
    echo "<script>location.replace('sf_bgimg.php');</script>";
}
                                   if (!empty($_GET)) {
                                       if ($_GET["delete"] =='true') {
$url = $_SERVER['DOCUMENT_ROOT']."/erp/uploads/us_bg/$bgimg";
            ...
            unlink("$url");
            $sql = "UPDATE usmf SET us_bgimg = NULL WHERE us_user = '$usernm'";
            $conn->query($sql);
               echo "<script>location.replace('sf_bgimg.php');</script>";
        }
                                       }
