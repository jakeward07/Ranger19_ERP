...
<h1>Sales Order Entry</h1>
   ...
   <?php if ($x ==$count) {} else {?> <a href="oe_lines.php?update=true&o=<?php echo $o?>&seq=<?php echo $x+1;?>"><button type="button">&rarr; Next Line</button></a><?php } ?>
    ...
    <div class="left">
        ...
        <p1>Price: </p1><input onkeyup="calcNet()" onfocus="fetchLast();popFields()" type="number" step="0.0001" id="netPrice" name="ln_price" required autocomplete="off"><br>
        ...
        <p1>Flag: </p1><input id="priceFlag" disabled>
         ...
         <?php } 
            ...
            if (!empty($_GET["update"])) {
                if ($_GET["update"] =='true') {
                   ?>
        <div class="bodystrip">
           <a href="oe_lines.php?o=<?php echo $o;?>"><button type="button">Add New Line</button></a>
    <?php if ($x ==0) {} else {?><a href="oe_lines.php?update=true&o=<?php echo $o?>&seq=<?php echo $x-1;?>"><button type="button">&larr; Previous Line</button></a> <?php }?>
   <?php if ($x ==$count) {} else {?> <a href="oe_lines.php?update=true&o=<?php echo $o?>&seq=<?php echo $x+1;?>"><button type="button">&rarr; Next Line</button></a><?php } ?>
    <a href="oe_inv.php?o=<?php echo $o?>"><button type="button">Invoice Order</button></a>
    <a href="javascript:void()"><button type="button" onclick="document.getElementById('oNotes').style.display = 'block'; return false;" title="These notes will appear on back order reports">Add Order Notes</button></a>
    ...
    <form id="lnForm" action="" onsubmit="calcMargin()" method="post">
    <! Loader div !>
          <div class="loading" id="loading" hidden><div class="loader"></div>
    <h4 style="text-align:center">Adding Line... Updating Inventory...</h4>
    ...
    <div class="left">
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) { 
                   ...
                   $lnId = $row["ln_id"];
                    ...
                    ?>
        <p1>Product Code: </p1><input id="im_sku" name="im_sku" disabled value="<?php echo $ln_sku?>" autocomplete="off">
        <br>
        ...
        <p1>Order Quantity: </p1> <input max="100000" onkeyup="setAlloc();popFields(); calcNet()" onblur="popFields()" id="orderQty" autofocus onfocus="this.select()" value="<?php echo $ln_oqty?>" name="ln_oqty" type="number" step="0.0001" autocomplete="off" required><br>
        ...
        <p1>Price: </p1><input value="<?php echo $ln_listprice?>" onkeyup="calcNet(); calcMargin()" onfocus="fetchLast()" type="number" step="0.0001" id="netPrice" name="ln_price" required autocomplete="off"><br>
...
$sql="SELECT * FROM imwh w INNER JOIN immf f ON f.im_id = w.im_id WHERE f.im_sku = '$sku[$x]' AND w.wh_site = $site_cd "; 
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        $avgc = $row["wh_avgcst"];
...
<?php if (!empty($row["wh_loc"])) {?><p1>Bin Location: </p1><input value="<?php echo $row["wh_loc"];?>" disabled><br><?php } ?>
            <input hidden id="margCs" value="<?php if ($avgc > 0) {echo $avgc;} else {echo $stdc;}?>"><br>
