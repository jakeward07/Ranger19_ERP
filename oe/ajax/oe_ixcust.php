...
$sql="SELECT * FROM invh h INNER JOIN cumf c ON c.cu_id = h.vh_cust INNER JOIN stmf s ON s.st_code = h.vh_site WHERE h.vh_cust = $q AND h.vh_site = $site_cd"; 
...
?>
   <tr class="tr">
    ...
    <td class="td">$<?php echo $row["vh_amtinc"];?></td>
       <td class="td"><a href="/oe/enq/oe_invdet.php?i=<?php echo $row["vh_inv"];?><?php if ((($sec =="ADM") or ($sec =="AR") or ($sec =="ARS"))) {echo "&s=$row[vh_site]";}?>">View Invoice</a></td>
    
    </tr>
    
    <?php  }} else {
        echo "<h1>No Invoice exists for $q</h1>";
    }
    ?>