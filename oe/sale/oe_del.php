...
<div class="left">
    ...
    <p1>Bill-to Account: </p1><input disabled><br>
    ...
    if ($result->num_rows > 0) {
        ...
        $sql5 = "INSERT INTO invh (vh_inv, vh_order, vh_site, vh_user, vh_cust, vh_amtex, vh_amtinc, vh_stampuser, vh_po, vh_job) (SELECT $inv, h.oh_order, h.oh_site, h.oh_user, h.oh_cust, '$_POST[ordex]', '$_POST[ordinc]', '$usernm', h.oh_cupo, h.oh_cujb FROM orhd h WHERE h.oh_site = '$site_cd' AND h.oh_order = '$_GET[o]')";
            $conn->query($sql5);
            //INSERT INTO INVL
            $sql7 = "INSERT INTO invl (vl_inv, vl_order, vl_site, vl_sku, vl_desc, vl_price, vl_incgst, vl_qty, vl_stampuser, vl_oqty, vl_bor) (SELECT '$inv', '$_GET[o]','$site_cd', ln_sku, ln_desc, ln_netprice, ln_netprice*1.1, ln_alloc, '$usernm', ln_oqty, ln_bor FROM orln WHERE ln_order = '$_GET[o]' AND ln_site = '$site_cd')";
            $conn->query($sql7);
            //Insert into ARBL
            $period = date('m-Y');
            $sql9 = "INSERT INTO arbl (bl_cust, bl_inv, bl_site, bl_amt, bl_period, bl_stampuser) (SELECT vh_cust, '$inv', '$site_cd', vh_amtinc, '$period', '$usernm' FROM invh WHERE vh_inv = '$inv' AND vh_site = $site_cd)";
            $conn->query($sql9);
                //Insert into oedp
