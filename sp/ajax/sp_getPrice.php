...
$q = $_GET["q"]; //Customer
$p = $conn -> real_escape_string($_GET["p"]); //Product
$c = $_GET["c"]; //Quantity
//Set date
$date = date("Y-m-d");
...
$sql = "SELECT * FROM prhd h INNER JOIN prcs c ON c.cs_code = h.pr_code INNER JOIN prln l ON l.pl_code = h.pr_code WHERE c.cs_cust = $q AND l.pl_sku = '$p' AND h.pr_startdate >= $date AND pr_expdate >= $date AND h.pr_site = $site_cd LIMIT 1";
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
?> <input id="flagM"  value="Promotional Price Selected"><br>
<input name="price" id="gPrice"  autocomplete="off" required value="<?php echo $row["pl_price"];?>">
<?php
                }} else {
                //CHECK CONTRACTS
                $sql = "SELECT * FROM cthd h INNER JOIN ctln l ON l.cl_code = h.ct_contract INNER JOIN immf i ON i.im_sku = '$p' INNER JOIN imwh w ON w.im_id = i.im_id AND w.wh_site = $site_cd WHERE h.ct_cust = $q AND l.cl_sku = '$p' AND h.ct_startdate >= $date AND h.ct_expdate >= $date AND l.cl_bval <= $c AND w.wh_site = $site_cd ORDER BY l.cl_bval DESC LIMIT 1";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) { 
...
<?php                        }}}} else {
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) { 
                    ?>    <input id="flagM"   value="Standard Pricing Selected"><br><?php
...
<input name="price" id="gPrice"  autocomplete="off" required value="<?php echo number_format( $row["wh_avgcst"]/((100-$row["sl_pval"])/100),4, ".","");?>">
    <input id="flagM"   value="Customer Fallback Settings Applied."><br>
<?php } else {?>
...
<?php
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        ?>   <input id="flagM"   value="Customer Fallback Settings Applied.">
<?php
                  if ($fbt =="Discount") {
                      if ($type ==1) {
                          ?>
...
<?php } else {?>
<input name="price" id="gPrice"  autocomplete="off" required value="<?php echo number_format( $row["im_stdc"]/((100-$fbv)/100),4, ".","");?>">  
<?php                        }
